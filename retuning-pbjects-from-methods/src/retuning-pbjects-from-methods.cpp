...
Animal createAnimal(){
	// Not efficient way of returning the object. Two copies.
	Animal a = Animal();
	a.setProperties("duck", 17);
	return a ;
	// a will go out of scope on hitting }

}

Animal* CreateAnimalEfficient(){
	// efficient method of returning objects
	Animal * a = new Animal();
	a->setProperties("Efficient duck", 27);
	return a ;
}
int main() {
	...
	Animal * efficientAniml = CreateAnimalEfficient();
	efficientAniml->speak() ;
	delete efficientAniml ;
	return 0;
}
